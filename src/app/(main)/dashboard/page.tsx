...
export default function AdminPortal() {
  const [activeTab, setActiveTab] = useState<'inquiries' | 'applications' | 'jobs' | 'docs'>('inquiries')
      ...
      <div className="min-h-screen" style={{ backgroundColor: '#FAFAF8' }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
            {(['inquiries', 'applications', 'jobs', 'docs'] as const).map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-4 py-2 text-sm font-medium rounded-full border transition-colors capitalize whitespace-nowrap ${activeTab === tab
          ...
          {/* Message */}
          {message && (
            <div className={`mb-6 p-3 rounded-lg text-sm ${message.includes('Error') ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'
              }`}>
              {message}
            </div>
          )}
